...
**Current state:** Phase 1 complete — infrastructure provisioned, tooling exists (`config-sync`, `seed-data`, `simple-agent`). Zero application code for the 8 core services.
...
- [ ] **Configuration pattern**: `AddJsonFile("appsettings.json") + AddEnvironmentVariables()` — same as `simple-agent`
...
├── Middleware/
│   └── ExceptionHandler.cs       # Global ProblemDetails error handler
├── Dockerfile
└── chart/
    ├── Chart.yaml
    ├── values.yaml
...
| Health Checks | `AspNetCore.HealthChecks.CosmosDb` | latest |
| Resilience | `Microsoft.Extensions.Http.Resilience` | latest |
| Config | `Microsoft.Extensions.Configuration.Json` | 10.0.5 |
| Config | `Microsoft.Extensions.Configuration.EnvironmentVariables` | 10.0.5 |

### Azure Services
- **Cosmos DB (CRM account)** — 6 containers: Customers, Orders, OrderItems, Products, Promotions, SupportTickets
- **Key Vault** (indirectly via config-sync → appsettings.json)
...
### Configuration Keys
```
COSMOSDB_CRM_ENDPOINT    → Cosmos DB account endpoint
COSMOSDB_CRM_DATABASE    → Database name (e.g., "contoso-crm")
