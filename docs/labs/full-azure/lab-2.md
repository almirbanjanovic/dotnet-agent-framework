...
| **AITool (Foundry Toolbox)** | `Microsoft.Extensions.AI.AITool` | [`crm-agent/Services/CrmAgentFactory.cs`](../../../src/crm-agent/Services/CrmAgentFactory.cs), [`product-agent/Services/ProductAgentFactory.cs`](../../../src/product-agent/Services/ProductAgentFactory.cs) | Optional hosted MCP Toolbox integration. It is off by default (`foundry_toolbox_name` defaults to empty and is projected as `Foundry__ToolboxName`). Product Agent suppresses this toolbox for anonymous guest sessions so hosted tools cannot bypass the guest guardrail. |
...
2. **Tools are discovered, not coded.** Replacing `crm-mcp` with a different process that exposes the same tool names requires zero changes to `crm-agent`. Hosted Foundry Toolbox tools are opt-in and append after the MCP-discovered list.
