...
- The loop is closed end-to-end: every terminal `FinalAction` (auto-approve, operator approve/reject, timeout) is posted back to CRM API at `POST /api/v1/internal/tickets/{id}/refund-decision` ([`fraud-workflow/Services/CrmApiClient.cs`](../../../src/fraud-workflow/Services/CrmApiClient.cs)). The customer sees the resolution on their `/tickets` page; rejections leave the ticket open with a "needs more info" comment so they can follow up. On the Full Azure Track this callback is what makes the durability story matter — a checkpointed workflow can replay across pod restarts and still deliver exactly one ticket-status update per alert.
