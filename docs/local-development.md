...
by setting `ASPNETCORE_ENVIRONMENT=Local` for the spawned process. The Aspire
AppHost does this automatically — you only need to set it for manual runs.
...
```bash
ASPNETCORE_ENVIRONMENT=Local dotnet run --project src/crm-api
```
...
`appsettings.Local.json` (the .NET 9 SDK's `--environment NAME=VALUE` flag
does **not** set this for you):
...
```bash
# Bash / macOS / Linux
ASPNETCORE_ENVIRONMENT=Local dotnet run --project src/crm-api --no-restore
```
