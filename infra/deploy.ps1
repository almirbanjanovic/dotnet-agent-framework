...
.DESCRIPTION
    ...
    Reads deployment configuration from deploy.env (generated by init.ps1)
...
$DeployEnvFile = "$ScriptDir\deploy.env"

if (-not (Test-Path $DeployEnvFile)) {
    throw "deploy.env not found. Run init first: ./init.ps1"
}

# Parse deploy.env key=value pairs
$deployConfig = @{}
...
if (-not $Environment -or -not $Location -or -not $BaseName -or -not $ResourceGroup) {
    throw "deploy.env is incomplete. Re-run init: ./init.ps1"
}
...
Write-Host ""
Write-Host "    Deployment Configuration (from deploy.env)" -ForegroundColor White
