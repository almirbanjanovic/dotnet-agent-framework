...
.DESCRIPTION
    ...
    Reads deployment configuration from infra/deployments/ (generated by init.ps1)
    and backend.hcl for the storage account.
...
if (-not (Test-Path $DeploymentsDir)) {
    throw "No deployments found. Run init first: ./init.ps1"
}

$envFiles = @(Get-ChildItem -Path $DeploymentsDir -Filter "*.env" | Sort-Object Name)
...
if ($envFiles.Count -eq 1) {
    $DeployEnvFile = $envFiles[0].FullName
} else {
    Write-Host ""
    ...
    for ($i = 0; $i -lt $envFiles.Count; $i++) {
        ...
        $parts = $fName -split '-', 2
        $envName = $parts[0]
        $regionName = if ($parts.Length -gt 1) { $parts[1] } else { "unknown" }
        # Parse timestamp from first comment line
        $dateStr = ""
        $firstLine = Get-Content $f.FullName -TotalCount 1
        if ($firstLine -match '(\d{4}-\d{2}-\d{2})') { $dateStr = $Matches[1] }
        $label = "$envName — $regionName"
        $num = $i + 1
        Write-Host ("     {0,2}) {1,-25} ({2})" -f $num, $label, $dateStr) -ForegroundColor Cyan
    }
    Write-Host ""
    $selection = Read-Host "    Select deployment [1]"
    if ([string]::IsNullOrWhiteSpace($selection)) { $selection = "1" }
    $idx = [int]$selection - 1
    if ($idx -lt 0 -or $idx -ge $envFiles.Count) {
        throw "Invalid selection: $selection"
    }
    $DeployEnvFile = $envFiles[$idx].FullName
}

# Parse deployment config key=value pairs
$deployConfig = @{}
Get-Content $DeployEnvFile | ForEach-Object {
    if ($_ -match '^\s*([A-Z_]+)\s*=\s*(.+)\s*$') {
        $deployConfig[$Matches[1]] = $Matches[2]
    }
}

$Environment    = $deployConfig["ENVIRONMENT"]
$Location       = $deployConfig["LOCATION"]
$BaseName       = $deployConfig["BASE_NAME"]
$ResourceGroup  = $deployConfig["RESOURCE_GROUP"]

if (-not $Environment -or -not $Location -or -not $BaseName -or -not $ResourceGroup) {
    throw "Deployment config is incomplete. Re-run init: ./init.ps1"
}
...
if (-not $StorageAccount) {
    throw "Could not read storage_account_name from backend.hcl. Re-run init.ps1."
}
...
Write-Host "    Region:         " -NoNewLine; Write-Host "$Location" -ForegroundColor Cyan
Write-Host "    Base Name:      " -NoNewLine; Write-Host "$BaseName" -ForegroundColor Cyan
...
if ($confirm -and $confirm -notmatch '^[Yy]') {
    Write-Host "    Aborted. Re-run init to change settings: ./init.ps1" -ForegroundColor Yellow
    exit 0
}
...
# Agent Identity sponsor: the signed-in user (Graph beta requires a User sponsor,
# ServicePrincipal objects are not valid for the sponsors reference)
if ($SpClientId) {
    $SpObjectId = az ad sp show --id "$SpClientId" --query id -o tsv 2>$null
    if ($SpObjectId) {
        Write-Info "SP object ID: $($SpObjectId.Trim()) (not used as sponsor — must be User)"
    }
    # Use the signed-in deployer's user OID as the blueprint sponsor
    $DeployerUserOid = az ad signed-in-user show --query id -o tsv 2>$null
    if ($DeployerUserOid) {
        $env:TF_VAR_agent_identity_sponsor_id = $DeployerUserOid.Trim()
        Write-Done "Agent Identity sponsor (user): $($DeployerUserOid.Trim())"
    } else {
        Write-Info "Could not determine signed-in user — blueprints may fail if sponsor is required"
    }
}
...
# ── Purge soft-deleted resources before plan ─────────────────────────────────
# Cognitive Services and Key Vaults use soft-delete by default. If a prior
# deployment was destroyed, the soft-deleted resources block re-creation.
Write-Step "Purging soft-deleted resources (if any)..."

$softCog = az cognitiveservices account list-deleted `
    --query "[?contains(id, '$ResourceGroup')].[name]" -o tsv 2>$null
foreach ($acct in ($softCog -split "`n" | Where-Object { $_ })) {
    $acct = $acct.Trim()
    az cognitiveservices account purge --location $Location `
        --resource-group $ResourceGroup --name $acct 2>$null | Out-Null
    Write-Done "Purged Cognitive Services: $acct"
}

$softKv = az keyvault list-deleted `
    --query "[?properties.vaultId && contains(properties.vaultId, '$ResourceGroup')].[name]" -o tsv 2>$null
foreach ($kv in ($softKv -split "`n" | Where-Object { $_ })) {
    $kv = $kv.Trim()
    az keyvault purge --name $kv --no-wait 2>$null | Out-Null
    Write-Done "Purged Key Vault: $kv"
}

if (-not $softCog -and -not $softKv) {
    Write-Info "No soft-deleted resources to purge"
}
