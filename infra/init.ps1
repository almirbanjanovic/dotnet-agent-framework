...
# ── Write deployment config (consumed by deploy scripts) ─────────────────────
$DeploymentsDir = "$ScriptDir\deployments"
if (-not (Test-Path $DeploymentsDir)) { New-Item -ItemType Directory -Path $DeploymentsDir -Force | Out-Null }
$DeployEnvPath = "$DeploymentsDir\$GitHubEnv-$Location.env"
$timestamp = Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"
Set-Content -Path $DeployEnvPath -Encoding UTF8 -Value @"
# Generated by init — $timestamp
# Environment: $GitHubEnv | Region: $Location
ENVIRONMENT=$GitHubEnv
LOCATION=$Location
BASE_NAME=$BaseName
RESOURCE_GROUP=$ResourceGroup
"@
Write-Done "deployments/$GitHubEnv-$Location.env"
