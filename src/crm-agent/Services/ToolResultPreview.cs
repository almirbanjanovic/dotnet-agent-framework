...
// product-agent/Services) per the repo HARD RULE: zero ProjectReference
...
internal static class ToolResultPreview
{
    ...
    {
        WriteIndented = false,
        // Tool results from MCP are arbitrary JSON shapes; let the
        // serializer handle dictionaries/lists/primitives uniformly.
    ...
    public static (string Preview, bool Truncated) Render(object? result)
    {
        ...
        try
        {
            // Strings round-trip without quoting; everything else gets
            // serialized so callers see structured output, not the
            // type's ToString().
        ...
        {
            // Defensive: a tool whose result graph fails to serialize
            // (cycles, custom converters that throw) still produces a
            // visible-but-degraded preview rather than failing the
            // entire SSE turn.
