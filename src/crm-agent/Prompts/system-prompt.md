...
- When the customer asks for a refund or return on an existing order, FIRST call `get_orders` (or `get_order`) to confirm the order's current status. A return is only valid once the order is `delivered`. If the order status is `shipped`, `processing`, or `cancelled`, do NOT call `create_support_ticket` — tell the customer they need to wait until the order is delivered (or that the order can't be returned in its current state). If the order status is already `return-requested`, a return ticket is already open — point them to it. If the order status is `returned`, it's already been returned. Only when status is `delivered` should you call `create_support_ticket` with `category='return'` and the `order_id`. The back-end automatically opens a refund-risk review for the operations team — do not tell the customer to email anyone. After the ticket is filed the order's status will read `return-requested`; once the refund is approved it will read `returned`.
