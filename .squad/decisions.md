...
# Decision: Fix deployer IP firewall cleanup — remove `--no-wait`, handle CIDR mismatch
...
After `deploy.ps1`/`deploy.sh` completed, the deployer IP remained in AI Search, Cosmos DB (both accounts), and Foundry firewalls. The `Remove-DeployerFirewallRules` function ran (in the `finally` block) but three bugs prevented effective cleanup.

## Decision

Fixed three root causes in both `deploy.ps1` and `deploy.sh`:

1. **Removed `--no-wait` from `az cosmosdb update`** in both Add and Remove functions. The async flag made removal fire-and-forget — Azure didn't finish processing before the script exited.

2. **Made IP filters CIDR-aware** for Cosmos DB and AI Search. String comparison `$_.Trim() -ne $DeployerIp` missed entries stored as `1.2.3.4/32`. Now filters both plain and CIDR formats.

3. **Added dual-format Foundry removal.** Previously only tried `--ip-address "$IP/32"`, but Terraform sets `ip_rules` with plain IPs. Now tries both `/32` and plain formats.

## Rationale

- Cosmos DB `--no-wait` comment said "firewall rule takes effect immediately" but the update still needs to complete server-side; exiting early means the old IP persists
- Azure normalizes IPs inconsistently across services: Cognitive Services may strip `/32`, Cosmos DB may add it
- Errors were suppressed (`2>$null`) so failures were invisible
- The cleanup position (finally block after all phases) is correct — no structural change needed

## Impact

- `infra/deploy.ps1` and `infra/deploy.sh` updated
- Cosmos DB updates now synchronous — adds ~30-60s to cleanup but guarantees completion
- No Terraform changes needed — the deployer IP is correctly managed via `allowed_ips` during apply and removed via az CLI post-apply
